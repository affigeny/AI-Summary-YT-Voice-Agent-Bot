...
| 2 | 🤖 yt-dlp Android VR | `player_client=android_vr` — без PO-токена, стабилен |
| 3 | 💧 Piped-зеркала | Публичные зеркала, проксирующие YouTube |
| 4 | 🔧 yt-dlp (web) | Стандартный клиент с браузерным User-Agent |
| 5 | 📺 yt-dlp TV-клиент | `player_client=tv` — не требует PO-токен |
| 6 | 📱 yt-dlp mWeb | `player_client=mweb` — мобильный веб-клиент |
| 7 | 🪞 Invidious-зеркала | Публичные зеркала (список проверяется, env `INVIDIOUS_INSTANCES`) |
| 8 | 🍪 yt-dlp + куки | Куки аккаунта (env `YT_COOKIES` или файл) |
