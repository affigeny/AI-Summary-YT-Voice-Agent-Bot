...
| 2 | 🔧 yt-dlp (web) | Стандартный клиент с браузерным User-Agent |
| 3 | 📺 yt-dlp TV-клиент | `player_client=tv` — не требует PO-токен, главный обход bot-check для датацентровых IP (Render) |
| 4 | 🤖 yt-dlp Android VR | `player_client=android_vr` — аналогично |
| 5 | 🪞 Invidious-зеркала | Публичные зеркала, проксирующие YouTube |
| 6 | 💧 Piped-зеркала | Ещё один тип зеркал |
| 7 | 🍪 yt-dlp + куки | Куки аккаунта (env `YT_COOKIES` или файл) |

Если субтитров нет вообще — бот скачивает аудио (TV-клиент → web → куки) и распознаёт речь локально через Whisper.

`/bypass` — закрепить конкретный метод или прогнать «🧪 Тест всех методов» с отчётом.
